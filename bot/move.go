...
func (bot *Bot) Move(m *discordgo.MessageCreate, params []string) (string, error) {
	...
	if numParams == 2 {
		...
		if err != nil {
			_, _ = bot.MoverSession.ChannelMessageSend(m.ChannelID, bot.Messages.CantFindChannel(utils.GetGuildLocale(bot.DB, m.GuildID), params[2]))
			return "", err
		}
		...
		_, _ = bot.MoverSession.ChannelMessageSend(m.ChannelID, bot.Messages.MoveHelper(utils.GetGuildLocale(bot.DB, m.GuildID), bot.Prefix, utils.ListChannelsForHelpMessage(guildChannels)))
		return "0", err
	}
}
