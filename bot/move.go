...
func (bot *Bot) Move(m *discordgo.MessageCreate, params []string) (string, error) {
	...
	if numParams == 2 {
		...
		if err != nil {
			_, _ = bot.MoverSession.ChannelMessageSend(m.ChannelID, fmt.Sprintf(bot.Messages[utils.GetGuildLocale(bot.DB, m)]["CantFindChannel"], params[1]))
			return "", err
		}

		if !utils.CheckPermissions(bot.MoverSession, destination, m.Author.ID, discordgo.PermissionVoiceMoveMembers) {
			_, _ = bot.MoverSession.ChannelMessageSend(m.ChannelID, bot.Messages[utils.GetGuildLocale(bot.DB, m)]["NoPermissionsDestination"])
			return "", err
		}
		...
		if err != nil {
			if err.Error() == "no permission origin" {
				...
				_, _ = bot.MoverSession.ChannelMessageSend(m.ChannelID, fmt.Sprintf(bot.Messages[utils.GetGuildLocale(bot.DB, m)]["SorryBut"], err.Error()))
			}
			return "", err
		}

		_, _ = bot.MoverSession.ChannelMessageSend(m.ChannelID, fmt.Sprintf(bot.Messages[utils.GetGuildLocale(bot.DB, m)]["JustMoved"], num))
		return num, err
	} else if numParams == 3 {
		...
		if err != nil {
			_, _ = bot.MoverSession.ChannelMessageSend(m.ChannelID, fmt.Sprintf(bot.Messages[utils.GetGuildLocale(bot.DB, m)]["CantFindChannel"], params[2]))
			return "", err
		}
		...
		if err != nil {
			_, _ = bot.MoverSession.ChannelMessageSend(m.ChannelID, fmt.Sprintf(bot.Messages[utils.GetGuildLocale(bot.DB, m)]["JustMoved"], err.Error()))
		...
		_, _ = bot.MoverSession.ChannelMessageSend(m.ChannelID, fmt.Sprintf(bot.Messages[utils.GetGuildLocale(bot.DB, m)]["MoveHelper"], bot.Prefix, bot.Prefix, bot.Prefix, utils.ListChannelsForHelpMessage(guildChannels)))
		return "0", err
	}
}
